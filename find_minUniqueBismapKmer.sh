...
if [[ ! -r $refrev ]]
then
  $BINDIR/refBismap $ref_untrimmed $refrev
fi

echo 'Bisulfite-treated reverse-complimented reference: ' $refrev
...
then
  $BINDIR/minUniqueKmer -b -t$threads -p$ref_untrimmed $ref_untrimmed $sa
fi
